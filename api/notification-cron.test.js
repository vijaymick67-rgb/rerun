...
import { createNotificationCronHandler } from './notification-cron.js'
...
describe('notification cron endpoint', () => {
  ...
  it('preserves rewrites and configures exactly the intended UTC cron', async () => {
    ...
    expect(vercel.crons).toEqual([{ path: '/api/notification-cron', schedule: '35 16 * * *' }])
  })

  it('contains no duplicated planner, release, platform, or timezone constants', async () => {
    const source = await readFile(new URL('./notification-cron.js', import.meta.url), 'utf8')
  ...
  it('relies on existing claims to prevent duplicate reminder sends across scheduler invocations', async () => {
    ...
    const notification = {
      ...
      title: 'Lucky â€” New episode', body: 'S1E1 Â· Episode', attachment: null,
